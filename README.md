...
检索层有没有效果，不该靠感觉。仓库内置一套可复现评测：固定随机种子生成评测集 → 多配置消融对比 → 输出 Hit@k / Recall@k / MRR。

```bash
# 1) 生成评测集（200 条，种子固定，任何机器结果一致）
python scripts/build_eval_set.py --n 200

# 2) 跑评测（--verify-online 读取线上 rag_retriever.TITLE_WEIGHT，复现同一索引逐条比对 top1）
python scripts/eval_rag.py --verify-online --markdown eval_report.md
```

**评测集分两档难度**：

| 档位 | query 来源 | 衡量什么 | 需要 API |
| --- | --- | --- | --- |
| easy（默认） | 语料原始标题 | **索引配置消融**：隔离测量「标题信号」的增益 | 否，零成本 |
| hard（`--llm`） | DeepSeek 改写的口语化提问（刻意避开原文用词） | **真实泛化**：用户真实问法下的检索效果 | 是 |

> ⚠️ easy 档的 query 与文档标题同源，绝对分数偏高，**不能当作真实泛化能力对外宣称**——它的价值在于消融对比（同一评测集下横向比配置），而非绝对值。真实泛化请跑 hard 档。

### 消融结果（200 条评测集 · 5000 chunks 语料）

| 配置 | Hit@1 | Hit@3 | Hit@5 | Hit@10 | Recall@5 | MRR@10 |
| --- | --- | --- | --- | --- | --- | --- |
| content-only（原始实现 · 历史对照） | 0.310 | 0.405 | 0.485 | 0.535 | 0.477 | 0.380 |
| title×1 + content | 0.865 | 0.975 | 0.995 | 1.000 | 0.995 | 0.922 |
| **title×3 + content（线上现行）** | **0.980** | **1.000** | **1.000** | **1.000** | **1.000** | **0.990** |
| title×5 + content | 0.990 | 1.000 | 1.000 | 1.000 | 1.000 | 0.995 |

**发现**：原始实现建索引时只对正文分词、**丢弃了标题信号**，Hit@5 仅 0.485；把标题以 ×3 权重并入索引后达到 1.000，MRR 从 0.380 提升到 0.990。**该改造已合入线上**（`app/rag_retriever.py` 的 `TITLE_WEIGHT=3`，可用环境变量 `RAG_TITLE_WEIGHT=0` 退回对照）。选 ×3 而非 ×5：Hit@5 均已到顶，×5 只多换来 MRR +0.005，而权重越高越易被标题词带偏。

评测脚本支持任意权重扫描（`--sweep 0,1,3,5,10`）、失败样例诊断（`--show-fails N`）与 markdown 导出（`--markdown`）。
...
**关键前提**：线上 `retrieve(top_k=4)` 只返回 4 个分数，归一化只能用这 4 个的**内部**统计量。用全库 mean/std 算的 z-score 看着更强（AUC 0.986），但**线上复现不了**，只是离线幻觉。下表才是线上真正可得的信号：

| 候选信号（top_k=4 内部，线上可得） | content-only（历史） | title×3（线上现行） |
| --- | --- | --- |
| `ratio = top1 / mean(top_k)` | 0.752 | **0.982** |
| `gap = top1 − top2` | 0.666 | 0.965 |
| `top1` 绝对分（原实现所用） | 0.517 | 0.894 |
| `z = (top1 − mean) / std` | 0.549 | 0.829 |

**根因**：原实现用 `sigmoid(top_bm25 / 3.0)` 做相似度分量，但 `BM25Okapi` 原始分实测在 10~120 量级，分母 3.0 让分量**恒饱和在 ~1.0**——库内与库外拿到几乎相同的分量，**AUC 0.517，等于抛硬币**；配套的「`top_bm25 ≤ 0` → 强制低置信」又因 BM25 分数恒为正而 **0/20 触发**（死代码）。也就是说：三层幻觉抑制里，**检索层的数值门控实际没有生效**，库外拦截全靠 LLM 自评那一层在扛。

z-score 在 `top_k` 内部还会被**封顶**在 √(k−1)≈1.73（分布为 [a,0,0,0] 时的上确界），不适合当查询内信号——真正可用的是 `ratio`。

**阈值选点**（FPR ≤ 10% 约束下保留率优先）：

| 信号 | 阈值 | TPR（库内保留） | FPR（库外漏过） |
| --- | --- | --- | --- |
| `ratio`（**已采用**） | ≥ 1.14 | **0.955**（191/200） | 0.100 |
| `gap` | ≥ 2.31 | 0.945（189/200） | 0.100 |
| `top1` 绝对分（原） | ≥ 28.6 | 0.735（147/200） | 0.100 |

**已合入线上**：`butler_fusion.py` 的相似度分量改用 `ratio`，并新增检索层硬门控 `ratio < 1.14 → 强制置信度 ≤ 0.20`（替代原死代码规则）。效果：

- 库内问题的**误拦率从 51.0% 降到 4.5%**
- 库外问题有了**独立于 LLM 自评**的拦截依据——检索层成为一道真正独立工作的闸门，而非装饰
...
- [x] 置信度门控校准：检索分量改用查询内 `ratio` 信号（AUC 0.517 → 0.982）+ 阈值选点 + 检索层硬门控落地
- [ ] 多框架适配：Dify / 自研 Agent
